...
현재 운영안에서는 별도 스냅샷 파일을 만들지 않는다.

이유:

- GitHub Actions가 하루 여러 번 실행되면 저장소에 파일이 계속 쌓인다.
- 신규 공고 판단에는 직전 실행 결과인 `data/*-latest.json`만 있으면 충분하다.
- 디버깅이 필요하면 Git 커밋 히스토리와 Actions 로그를 우선 활용한다.
...
그래서 `latest.json` 보관과 workflow 실패 알림이 중요하다.
